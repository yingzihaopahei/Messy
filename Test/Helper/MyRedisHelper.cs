...
namespace FPress.Web.Models.MyRedis
{
    public class MyRedisHelper
    {
        private static string[] ReadWriteHosts = ConfigurationManager.AppSettings["RedisHosts"].Split(new char[] { '|' });

        #region -- 连接信息 --
        public static PooledRedisClientManager prcm = CreateManager(ReadWriteHosts);

        private static PooledRedisClientManager CreateManager(string[] readWriteHosts)
        {
            // 支持读写分离，均衡负载  
